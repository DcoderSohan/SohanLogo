...
const StatFormModal = ({ isOpen, onClose, data, onSave }) => {
  const [formData, setFormData] = useState({ label: "", value: 0, plus: false });

  useEffect(() => {
    if (data) {
      setFormData({ label: data.label || "", value: data.value || 0, plus: data.plus || false });
    } else {
      setFormData({ label: "", value: 0, plus: false });
    }
  }, [data, isOpen]);
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[60] p-4 pointer-events-none">
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-white/10">
                <h2 className="text-xl font-bold">{data ? "Edit Stat" : "Add Stat"}</h2>
                <button
                  onClick={onClose}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, plus: e.target.checked })}
                    className="w-4 h-4"
                  />
                  <label className="text-sm">Show Plus Sign (+)</label>
                </div>
